...
test("eslint complexity remains measured as a non-blocking warning", () => {
  assert.match(packageSource, /"lint": "eslint"/);
  assert.match(
    eslintConfigSource,
    /complexity:\s*\[\s*"warn",\s*\{\s*max:\s*10\s*\}\s*\]/
  );
  assert.doesNotMatch(eslintConfigSource, /complexity:\s*\[\s*"error"/);
});
