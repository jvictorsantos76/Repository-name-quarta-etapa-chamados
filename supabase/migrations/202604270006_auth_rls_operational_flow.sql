...
begin
  ...
  if to_regclass('public.registros_tecnicos') is not null then
...
begin
  ...
  if to_regclass('public.historico_status') is not null
    and to_regprocedure('public.usuario_operacional_ativo()') is not null
    and not exists (
      select 1 from pg_policies
      where schemaname = 'public'
  ...
  if to_regclass('public.registros_tecnicos') is not null
    and to_regprocedure('public.usuario_operacional_ativo()') is not null
    and not exists (
      select 1 from pg_policies
      where schemaname = 'public'
