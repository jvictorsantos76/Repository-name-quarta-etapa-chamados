...
type HorarioAtendimentoSerializado = {
  dia_semana: number;
  fechado: boolean;
  abre_as: string | null;
...
function horarioCurto(valor: string | null | undefined) {
  const texto = String(valor ?? "").trim();
  return /^\d{2}:\d{2}:\d{2}$/.test(texto) ? texto.slice(0, 5) : texto;
}

function criarAgendaAtendimentoPadrao(): DiaAtendimento[] {
...
function montarAgendaAtendimentoInicial(
  horarios: ParceiroHorarioAtendimento[] | undefined
) {
  if (!horarios?.length) {
...
function serializarAgendaAtendimento(
...
function horarioEmMinutos(valor: string) {
...
function formatarHorarioMinutos(totalMinutos: number) {
  const horas = Math.floor(totalMinutos / 60).toString().padStart(2, "0");
  const minutos = (totalMinutos % 60).toString().padStart(2, "0");
  return `${horas}:${minutos}`;
}

function criarIntervaloAdicional(intervalos: IntervaloAtendimento[]) {
...
function validarAgendaAtendimento(agenda: DiaAtendimento[]) {
...
function AgendaSemanalAtendimento({
  agenda,
  onChange,
  erro,
}: {
                      ...
                      {dia.intervalos.map((intervalo, index) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            className="mt-5 inline-flex min-h-8 items-center justify-center rounded-md border border-gray-300 px-3 text-xs font-semibold text-gray-700 transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:text-gray-300"
                          >
                            Remover
                          </button>
                        </div>
                ...
                <td className="px-3 py-3 text-center">
                  <button
                    ...
                    onClick={() =>
                      atualizarDia(dia.dia_semana, (atual) => ({
                        ...atual,
                        intervalos: [...atual.intervalos, criarIntervaloAdicional(atual.intervalos)],
                      }))
                    }
                    className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-gray-300 bg-white text-base font-bold text-gray-800 transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:text-gray-300"
                    aria-label={`Adicionar intervalo em ${dia.label}`}
                  >
                    +
                  </button>
                </td>
